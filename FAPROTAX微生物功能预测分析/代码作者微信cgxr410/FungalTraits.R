VX_cgxr410_FungalTraits <- function(VXcgxr410) {
  if (!dir.exists("18s-ITS")) {
    dir.create("18s-ITS")
  }
  ...
  # 保存图像和最终结果
  ggsave('18s-ITS/1.plot_FungalTraits.pdf', plot = p, width = width1, height = height1)
  write.csv(t2$res_spe_func_perc, file = "18s-ITS/result_FungalTraits.csv", row.names = TRUE)
  ...
  # 打印图形
  ggsave('18s-ITS/2.boxplot_FungalTraits.pdf', plot = p1, width = width2, height = height2)
  return(result_FungalTraits=result_FAPROTAX)
}
